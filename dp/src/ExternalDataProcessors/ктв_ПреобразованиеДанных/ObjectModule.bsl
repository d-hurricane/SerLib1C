...
//
// Возвращаемое значение:
//   Произвольный, Структура            - результат преобразования
//
...
Функция ОбработчикЗаписиЗначенийJSON(Свойство, Значение, ДополнительныеПараметры, Отказ) Экспорт
	...
	Если ТипЗначения = Тип("ДвоичныеДанные") Тогда
		Значение = СтрШаблонВнутр("/BinaryData(%1)/", Base64Строка(Значение));
...
Функция ОбработчикЧтенияЗначенийJSON(Свойство, Значение, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		Если СтрокаНачинаетсяСВнутр(Значение, "/Date(") Тогда
			...
			КонецЕсли;
		ИначеЕсли СтрокаНачинаетсяСВнутр(Значение, "/BinaryData(") Тогда
...
//                Ложь   - в противном случае
//
Функция ЭтоЗначениеСКД(Значение)
...
//        **ДопустимыйЗнак              - Строка            - Допустимый знак
//     *КвалификаторыСтроки           - Структура         - Описание квалификаторов строки:
//        **Длина                       - Число             - Длина
//        **ДопустимаяДлина             - Строка            - Разрядность дробной части
//     *КвалификаторыДаты             - Структура         - Описание квалификаторов даты:
//        **ЧастиДаты                   - Строка          - Части даты (Дата / Время)
...
Функция ОписаниеТиповВСтруктуру(ОписаниеТипов) Экспорт
	...
	Если ОписаниеТипов.СодержитТип(Тип("Число")) Тогда
		...
		СтруктураКвалификаторов = Новый Структура();
		СтруктураКвалификаторов.Вставить("Разрядность", Квалификаторы.Разрядность);
		СтруктураКвалификаторов.Вставить("РазрядностьДробнойЧасти", Квалификаторы.РазрядностьДробнойЧасти);
		Если Квалификаторы.ДопустимыйЗнак =  ДопустимыйЗнак.Неотрицательный Тогда
			СтруктураКвалификаторов.Вставить("ДопустимыйЗнак", "Неотрицательный");
		Иначе
			СтруктураКвалификаторов.Вставить("ДопустимыйЗнак", "Любой");
		КонецЕсли;
		
		СтруктураОписаниеТипов.Вставить("КвалификаторыЧисла", СтруктураКвалификаторов);
	КонецЕсли;
	...
	Если ОписаниеТипов.СодержитТип(Тип("Строка")) Тогда
		...
		СтруктураКвалификаторов = Новый Структура();
		СтруктураКвалификаторов.Вставить("Длина", Квалификаторы.Длина);
		Если Квалификаторы.ДопустимаяДлина = ДопустимаяДлина.Фиксированная Тогда
			СтруктураКвалификаторов.Вставить("ДопустимаяДлина", "Фиксированная");
		Иначе
			СтруктураКвалификаторов.Вставить("ДопустимаяДлина", "Переменная");
		КонецЕсли;
		
		СтруктураОписаниеТипов.Вставить("КвалификаторыСтроки", СтруктураКвалификаторов);
	КонецЕсли;
	...
	Если ОписаниеТипов.СодержитТип(Тип("Дата")) Тогда
		...
		СтруктураКвалификаторов = Новый Структура();
		Если Квалификаторы.ЧастиДаты = ЧастиДаты.Дата Тогда
			СтруктураКвалификаторов.Вставить("ЧастиДаты", "Дата");
		ИначеЕсли Квалификаторы.ЧастиДаты = ЧастиДаты.Время Тогда
			СтруктураКвалификаторов.Вставить("ЧастиДаты", "Время");
		Иначе
			СтруктураКвалификаторов.Вставить("ЧастиДаты", "ДатаВремя");
		КонецЕсли;
		
		СтруктураОписаниеТипов.Вставить("КвалификаторыДаты", СтруктураКвалификаторов);
	КонецЕсли;

	Если ОписаниеТипов.СодержитТип(Тип("ДвоичныеДанные")) Тогда
		Квалификаторы = ОписаниеТипов.КвалификаторыДвоичныхДанных;
		...
		СтруктураОписаниеТипов.Вставить("КвалификаторыДвоичныхДанных", СтруктураКвалификаторов);
	КонецЕсли;
...
Функция ОписаниеТиповИзСтруктуры(СтруктураОписаниеТипов) Экспорт
	...
	Если СтруктураОписаниеТипов.Свойство("КвалификаторыЧисла") Тогда
		КвалификаторыЧисла = Новый КвалификаторыЧисла(
			СтруктураОписаниеТипов.КвалификаторыЧисла.Разрядность,
			СтруктураОписаниеТипов.КвалификаторыЧисла.РазрядностьДробнойЧасти,
			ДопустимыйЗнак[СтруктураОписаниеТипов.КвалификаторыЧисла.ДопустимыйЗнак]);
	КонецЕсли;
	
	КвалификаторыСтроки = Неопределено;
	Если СтруктураОписаниеТипов.Свойство("КвалификаторыСтроки") Тогда
		КвалификаторыСтроки = Новый КвалификаторыСтроки(
			СтруктураОписаниеТипов.КвалификаторыСтроки.Длина,
			ДопустимаяДлина[СтруктураОписаниеТипов.КвалификаторыСтроки.ДопустимаяДлина]);
	КонецЕсли;
	
	КвалификаторыДаты = Неопределено;
	Если СтруктураОписаниеТипов.Свойство("КвалификаторыДаты") Тогда
		КвалификаторыДаты = Новый КвалификаторыДаты(
			ЧастиДаты[СтруктураОписаниеТипов.КвалификаторыДаты.ЧастиДаты]);
	КонецЕсли;
	
	КвалификаторыДвоичныхДанных = Неопределено;
	Если СтруктураОписаниеТипов.Свойство("КвалификаторыДвоичныхДанных") Тогда
		КвалификаторыДвоичныхДанных = Новый КвалификаторыДвоичныхДанных(
			СтруктураОписаниеТипов.КвалификаторыДвоичныхДанных.Длина,
			ДопустимаяДлина[СтруктураОписаниеТипов.КвалификаторыДвоичныхДанных.ДопустимаяДлина]);
	КонецЕсли;
...
Функция КартинкаИзСтруктуры(СтруктураЗначения)
	...
	Если СтруктураЗначения.Свойство("Имя") И ЗначениеЗаполнено(СтруктураЗначения.Имя) Тогда
		Значение = БиблиотекаКартинок.Получить(СтруктураЗначения.Имя);
...
Функция ПланВидовХарактеристикВСтруктуру(Объект, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Если МетаОбъект.Иерархический И Не Объект.ЭтоГруппа Тогда
...
Процедура ПланВидовХарактеристикИзСтруктуры(Объект,
	...
	Если МетаОбъект.Иерархический И Не Объект.ЭтоГруппа
		И СтруктураОбъекта.ТипЗначения.Типы.Количество() > 0 Тогда
...
Процедура ПолеГруппировкиСКДИзСтруктуры(Объект, СтруктураОбъекта)
	...
	Если СтруктураОбъекта.Свойство("НачалоПериода") И ЗначениеЗаполнено(СтруктураОбъекта.НачалоПериода) Тогда
		Если ТипЗнч(СтруктураОбъекта.НачалоПериода) = Тип("Структура") Тогда
			...
			Иначе
				ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип поля ""НачалоПериода"": %1", СтруктураОбъекта.НачалоПериода.__Тип));
			КонецЕсли;
	...
	Если СтруктураОбъекта.Свойство("КонецПериода") И ЗначениеЗаполнено(СтруктураОбъекта.КонецПериода) Тогда
		...
		Иначе
			ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип поля ""КонецПериода"": %1", СтруктураОбъекта.КонецПериода));
		КонецЕсли;
	КонецЕсли;
...
Функция ЗначениеСКДВСтруктуру(Объект)
	...
	Если ПрефиксИмениМетодаПреобразования = Неопределено Тогда
		ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип %1 объекта СКД: %2", ТипЗначения, Объект));
		Возврат Неопределено;
	КонецЕсли;
...
Функция СистемныеПеречисления()
	...
	МакетПеречислений = ЭтотОбъект.ПолучитьМакет("Перечисления");
...
Функция ТипыЗначенийСКД()
	...
	МакетТиповСКД = ЭтотОбъект.ПолучитьМакет("ТипыЗначенийСКД");
...
Функция Цвета()
	...
	МакетЦветов = ЭтотОбъект.ПолучитьМакет("Цвета");
...
Функция Шрифты()
	...
	МакетШрифтов = ЭтотОбъект.ПолучитьМакет("Шрифты");
...
Функция Рамки()
	...
	МакетРамок = ЭтотОбъект.ПолучитьМакет("Рамки");
...
Функция Картинки()
	...
	МакетКартинок = ЭтотОбъект.ПолучитьМакет("Картинки");
